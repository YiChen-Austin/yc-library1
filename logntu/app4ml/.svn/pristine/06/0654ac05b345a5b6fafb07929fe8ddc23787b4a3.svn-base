...
(function($) {
	...
	$.fn.validatebox.defaults = {
		...
		rules : {
			...
			phone : {
				validator : function(_1112) {
					//验证数字
				var reg = new RegExp("^0?\\d{11}$");
				if (!reg.test(_1112)) {
					return false;
				} else {
					return true;
			...
			message : "Please enter phoneNum."
			},
			
			ip : {
				validator : function(_1113) {
...
(function($) {
	...
	function _37d(year, _37e) {
		...
		while (_384.length < 7) {
			...
			_384.push( [ date.getFullYear(), date.getMonth() + 1,
					date.getDate() ]);
		}
		if (_381.length < 6) {
			var _385 = _384[_384.length - 1];
						...
						renderFooter : function(_638, _639, _63a) {
							...
							for ( var i = 0; i < rows.length; i++) {
								...
								_63c.push("<tr node-id=" + row[opts.idField]
										+ ">");
								_63c.push(this.renderRow.call(this, _638, _63b,
										_63a, 0, row));
								_63c.push("</tr>");
							}
