...
var validate = Class
		.extend({
			defaultCfg:{
				rules:{},
				submitFun:function(){},
				errorLabel:'<label style="color:#b91c21;position: absolute;bottom: 0px;left: 5px;"></label>',
				errorFun:function(){}
			},
			init:function(cfg){				
			...
			toShowError:function(ele,message){
				...
				if(!$(ele).closest(".form-group").attr("not-allow")){
					$(ele).after(error);
					$(ele).css("border","1px solid #b91c21").closest(".form-group")
...
var validateConstant = {
	...
	"sfz1":/^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/,// 身份证
