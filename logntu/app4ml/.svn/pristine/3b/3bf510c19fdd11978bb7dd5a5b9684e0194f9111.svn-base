$(function() {
	...
	$("#createButton").click(function() {
		window.location = $("#path",parent.document).val()+"/page/"+"createDesktopDefend";
	});
	// 修改功能
	$("#updateButton").click(function() {
		var gr = jQuery("#desktopDefendList").jqGrid("getGridParam", "selarrrow");
		if (gr == '' || gr.length>1) {
			jAlert("请选择要修改的记录!");
			return false;
		}else{
			gr = jQuery("#desktopDefendList").jqGrid('getGridParam', 'selrow');
			if (gr) {
				var rs = jQuery("#desktopDefendList").jqGrid('getRowData', gr);
				window.location = $("#path",parent.document).val()+"/page/"+"findDesktopDefend?id=" + rs.id;
			}			
		}
	});
	// 删除功能
	$("#deleteButton").click(function() {
		...
		if (gr == '') {
			...
			jQuery("#desktopDefendList").jqGrid('setGridParam', {
				type : "post",
				datatype : "json",
				url : $("#path",parent.document).val()+"/page/"+"deleteDesktopDefends?deleteIDs=" + gr,
				page : 1
			}).trigger("reloadGrid");
		}
	});
