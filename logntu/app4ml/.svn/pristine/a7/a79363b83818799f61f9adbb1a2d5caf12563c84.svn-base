...
public class TreeUtil {
	...
	private static TreeNodeBean entityConvertTreeNodeBean(Object entityNode) throws Exception {
		...
		//信息管理栏目数部分
		try {
			Method getAdministratorNameMethod = entityNode.getClass().getMethod("getAdministratorName", new Class[] {});
			String administratorName = (String) getAdministratorNameMethod.invoke(entityNode, new Object[] {});
			map.put("administratorName", BaseUtil.isEmpty(administratorName) ? "" : administratorName);
		} catch (Exception e) {
			map.put("administratorName", "");
		}

		try {
			Method getAdministratorIdMethod = entityNode.getClass().getMethod("getAdministratorId", new Class[] {});
			String administratorId = (String) getAdministratorIdMethod.invoke(entityNode, new Object[] {});
			map.put("administratorId", BaseUtil.isEmpty(administratorId) ? "" : administratorId);
		} catch (Exception e) {
			map.put("administratorId", "");
		}
		try {
			Method getPublisherNameMethod = entityNode.getClass().getMethod("getPublisherName", new Class[] {});
			String publisherName = (String) getPublisherNameMethod.invoke(entityNode, new Object[] {});
			map.put("publisherName", BaseUtil.isEmpty(publisherName) ? "" : publisherName);
		} catch (Exception e) {
			map.put("publisherName", "");
		}
		try {
			Method getPublisherIdMethod = entityNode.getClass().getMethod("getPublisherId", new Class[] {});
			String publisherId = (String) getPublisherIdMethod.invoke(entityNode, new Object[] {});
			map.put("publisherId", BaseUtil.isEmpty(publisherId) ? "" : publisherId);
		} catch (Exception e) {
			map.put("publisherId", "");
		}
