...
/*
 * 功能：改写的jsTree 1.0rc2 checkbox插件
 * 时间：2010.11.10 
 * 作者：xgyin
 */
(function ($) {
	$.jstree.plugin("mycheckbox", {
		__init : function () {
		...
		_fn : {
			...
			change_state : function (obj, state) {
				...
				if(state) { obj.find("li").andSelf().removeClass("jstree-checked jstree-undetermined").addClass("jstree-unchecked"); }
					...
					this.data.mycheckbox.last_selected = obj;
				}
				/**
				 * 2010.11.9 xgyin在农商行注释掉原有程序obj.parentsUntil(".jstree", "li")
				  obj.parentsUntil(".jstree", "li").each(function () {
					var $this = $(this);
					if(state) {
				 ...
				 */
				/**
				 * 2010.11.9 xgyin在农商行修改的obj.parentsUntil(".jstree", "li")
				 */
				obj.parentsUntil(".jstree", "li").each(function () {
					var $this = $(this);
					if(state) {
						$this.removeClass("jstree-checked jstree-undetermined").addClass("jstree-checked");						
					}
					else {
						if($this.children("ul").children(".jstree-unchecked, .jstree-undetermined").length) {
			...
			/**
			 * 2010.11.9 xgyin在农商行新增方法:仅仅选中指定的节点
			 */
			alone_checked: function(obj){
				 this._get_node(obj).find("ul").andSelf().removeClass("jstree-unchecked jstree-undetermined").addClass("jstree-checked");
				
			},
			/**
			 * 2010.11.9 xgyin在农商行新增方法:获取所有选中的节点ID集合
			 */
			get_all_checked : function() {
				var checkNodes = new Array();
				var nodes =  this.get_container().find("a:not(jstree-checked)").parent();
				for(var i=0;i<nodes.size();i++){
					var node = nodes[i];
					if(this.is_checked(node))
						checkNodes.push(this._get_node(node).attr("id"));
				}
				return checkNodes;
			},
			/**
			 * 2010.11.9 xgyin在农商行新增方法:获取所有未选中的节点
			 */
			get_all_unchecked : function() {
				var uncheckNodes = new Array();
				var nodes =  this.get_container().find("a:not(jstree-checked)").parent();
				for(var i=0;i<nodes.size();i++){
					var node = nodes[i];
					if(!this.is_checked(node))
						uncheckNodes.push(this._get_node(node));
				}
				return uncheckNodes;
			},
	...
	$.jstree.plugin("checkbox", {
		...
		_fn : {
			...
			change_state : function (obj, state) {
				...
				/** 2011-5-11注释原有程序obj.parentsUntil(".jstree", "li").each(function ()**/
				/**
				obj.parentsUntil(".jstree", "li").each(function () {
					var $this = $(this);
					if(state) {
				...
				**/
				/** 2011-5-11修改obj.parentsUntil(".jstree", "li").each(function ()**/
				obj.parentsUntil(".jstree", "li").each(function () {
					var $this = $(this);
					if(state) {
						if($this.children("ul").children(".jstree-checked, .jstree-undetermined").length) {
						...
						var $thisLength = $this.children("ul").children(".jstree-unchecked, .jstree-undetermined").length;
						var $orgTypeCode = obj.attr('orgTypeCode');
						if($thisLength) {
							$this.parentsUntil(".jstree", "li").andSelf().removeClass("jstree-checked jstree-unchecked").addClass("jstree-undetermined");
							return false;
						}
							...
							if($orgTypeCode=='总行部门')
								$this.removeClass("jstree-unchecked jstree-undetermined").addClass("jstree-undetermined");
							else{
								$this.removeClass("jstree-unchecked jstree-undetermined").addClass("jstree-checked");
							}
						}
			...
			/**
			 * 2010.12.27 xgyin在农商行新增方法:获取所有选中的节点ID集合
			 */
			get_all_checked : function() {
				var checkNodes = new Array();
				var nodes =  this.get_container().find("a:not(jstree-checked)").parent();
