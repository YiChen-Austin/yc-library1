...
<script type="text/javascript" src="<%=path%>/js/ux/system/business_dictionary_detail.js"></script>
...
<title>业务字典新增页面</title>
</head>
<body>

<div>
<form method="post" id="detailBusinessDictionary"><br/>
	...
	<table class="mainTable">
		<tr><td>
			...
			<input type="text" id="cnName" value="${sysDictionaryBean.cnName}" class="validate[required,custom[chinese],length[0,40]]" />
			<font color="red">*</font>
		</td><td>
			<label for="name">英文名称：</label>
		</td><td>
			<input type="text" id="enName" value="${sysDictionaryBean.name}" class="validate[required,custom[englishNumberLine],length[0,40]]" />
			<font color="red">*</font>
		</td></tr>
		<tr><td>
			<label for="flag">可否编辑：</label>
		</td><td>
			<input type="radio" value="1" name="businessDictionaryBean.flag" id="flag" checked="checked" />
			<label for="flag">是</label>
			<input type="radio" value="0" name="businessDictionaryBean.flag" id="flag" />
			<label for="flag">否</label>
			<font color="red">*</font>
		</td></tr>
		<tr><td>
			<label for="flag">备注：</label>
		</td><td colspan="3">
			<textarea id="remark" rows="4" cols="36">${sysDictionaryBean.remark }</textarea>
		</td></tr>
	</table>
