...
public class PkipairTest {
	...
	public String signMsg(String signMsg) {
		...
		try {
			...
			String file = Pkipair.class
					...
					.replaceAll("%20", " ");
			// String file =
			// PkipairTest.class.getResource("/keys/bill/99bill.rsa.pfx").toURI().getPath()
			// .replaceAll("%20", " ");
			...
			// char[] keyPwd = "1qaz4rfv".toCharArray();
	...
	public boolean enCodeByCer(String val, String msg) {
		...
		try {
			...
			// 获得文件(相对路径)
			String file = PkipairTest.class
					.getResource("/keys/bill/99bill[1].cert.rsa.20140803.cer")
					.toURI().getPath();
