...
<body>
	<div>
		<form:form action="modifySysUser" method="post" id="sysUser"
			commandName="sysUserBean">
			<br />
			<div id="errorMessage"></div>
			<div id="dateCheck" style="display: none" class="errorMessage"></div>
			<table class="mainTable">
				<tr>
					...
					<td><input type="text" name="realName" id="realName"
						...
						type="hidden" name="id" value="${sysUserBean.id }" /><input
				...
				<tr>
					...
					<td><input type="text" name="identityCard" id="identityCard"
						...
						class="validate[custom[IDCard]]" maxlength="18" /></td>
				</tr>
				<tr>
					<td><label for="isOperator">操作员：</label></td>
					<td><input type="radio" value="true" name="isOperator"
						id="isOperatorY" ${sysUserBean.isOperator==true?'checked':''}
						onclick="operatorCheck();" /><label for="isOperatorY">是 </label><input
						type="radio" name="isOperator" value="false" id="isOperatorN"
						${sysUserBean.isOperator==false?'checked':''}
						onclick="operatorCheck();" /><label for="isOperatorN" />否</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td align="left" colspan="3"><input type="hidden"
						...
						readonly="readonly" value="${sysUserBean.sysOrganizationIDs }"></input>
						<textarea name="orgNames" id="orgNames" rows=4 cols=40
							readonly="readonly">${sysUserBean.orgNames }</textarea></td>
				</tr>
				<tr>
					<td align="right">分配角色：</td>
					<td align="left" colspan="3"><input type="hidden"
						name="sysRoleIDs" id="sysRoleIDs" size="80" readonly="readonly"
						value="${sysUserBean.sysRoleIDs }"></input> <textarea
							name="roleName" id="roleName" rows=4 cols=40 readonly="readonly">${sysUserBean.roleName }</textarea></td>
				</tr>
			</table>
			<table>
				<tr>
					...
					<td colspan="2"><input type="button" value="保存"
						id="saveSubmit_m" style="margin-right: 5px" /><input type="reset"
						value="重置" onclick="resetSysUser()" id="resetSubmit"
