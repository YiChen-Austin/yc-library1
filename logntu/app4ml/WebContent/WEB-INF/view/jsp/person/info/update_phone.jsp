	...
	<div class="main w1100">
		...
		<div class="main-con fl">
			<div class="update-div">
				<img src="<%=path%>/images/update-one.png" >
				<ul class="update-ul">
					<li>
						您的账号当前账号是：
						<span id="old_phone">${session_mem_user.userName}</span>
					</li>
					<li >
...
function totime(){
	var useremail = {
			email:$("#old_phone").text(),
			code:$('#syzms').val(),
	    };
	var userphone = {
			phone:$("#old_phone").text(),
			code:$('#syzms').val(),
	    };
		...
		$.ajax({
         ...
         success:function(msg){
         	 if(msg.serviceCode==0){
         			layer.closeAll();
        		 	window.location.href ="<%=project%>/member/new_phone" + "?token=" + msg.token;
	           	}else{
	           		layer.closeAll();
	           		layer.msg('输入有误');
	           	}  
