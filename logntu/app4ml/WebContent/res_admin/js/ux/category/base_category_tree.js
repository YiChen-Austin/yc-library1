					...
					types : {
						...
						data : {
							...
							opts : {
								async : true,
								method : "POST",
								url : $("#path",parent.document).val()+"/category/findAllBaseCategorys"// 实际提交的Action
							}
						},
						lang : {
							...
							onselect : function(NODE, TREE_OBJ) {
								...
								if ($(NODE).attr("parentId")!= '') {
									...
									$('#secondaryLevelCreateButton')
											.removeAttr('disabled');
									//20140220
									$('#sameLevelCreateButton').removeClass("disableClass");
									$('#secondaryLevelCreateButton').removeClass("disableClass");
									if($(NODE).attr("flag")=='0'){
										$('#deleteButton').attr(
												'disabled', 'disabled');
										$('#saveButton').attr(
												'disabled', 'disabled');
										//20140220
										$('#deleteButton').addClass("disableClass");
										...
										//20140220
										$('#deleteButton').removeClass("disableClass");
									...
									$('#saveButton').attr(
											'disabled', 'disabled');
									//20140220
									$('#sameLevelCreateButton').addClass("disableClass");
									$('#secondaryLevelCreateButton').removeClass("disableClass");
									$('#deleteButton').addClass("disableClass");
									$('#saveButton').addClass("disableClass");
								}
...
//					}
				...
				var actionMethod = 'modifyBaseCategory';
				if(operatorType=='same')
					actionMethod = 'sameSaveBaseCategory';
				else if(operatorType=='secondary')
					actionMethod = 'secondarySaveBaseCategory';
				else 
					actionMethod = 'modifyBaseCategory';
				$.ajax( {
					async : false,
					url : $("#path",parent.document).val()+"/category/"+actionMethod,
					type : 'POST',
					dataType : 'json',
	...
	$("#deleteButton").click(function() {
		...
		$.ajax( {
			async : false,
			url : $("#path",parent.document).val()+'/category/deleteBaseCategory?id=' + node.id,
			type : 'POST',
			dataType : 'json',
			timeout : 1000,
			success : function(data) {
