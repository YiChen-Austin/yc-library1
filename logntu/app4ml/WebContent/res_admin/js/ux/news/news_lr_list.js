...
$(function() {
	...
	jQuery("#newsList").jqGrid({
		...
		datatype : "local",
		// 列显示名称，是一个数组对象
		colNames : [ 'ID' ,'封面','资讯标题','分类', '资讯摘要', '时间','发布机构'],
	...
	function toQuery() {
		...
		jQuery("#newsList").jqGrid('setGridParam', {
			type : "post",
			datatype : "json",
			url : $("#path",parent.document).val()+"/newsAdmin/findNewsAll"
		}).trigger("reloadGrid");
	}
	// 查询
	...
	$("#singleNewsButton").click(function() {
		...
		if (rowId == '' || rowId == null || rowId.length > 1) {
			...
			window.location = $("#path",parent.document).val()+"/newsAdmin/viewNews?id=" + rowId;
		}
	});
	// 添加
	$("#saveNewsButton").click(function() {
		window.location = $("#path",parent.document).val()+"/newsAdmin/createNews";
	});
	// 修改
	$("#updateNewsButton").click(function() {
		...
		if (rowId == '' || rowId == null || rowId.length > 1) {
			...
			window.location = $("#path",parent.document).val()+"/newsAdmin/updateNews?id=" + rowId;
		}
	});
	// 删除
	$("#deleteNewsButton").click(function() {
		...
		if (gr == '') {
			...
			$.ajax({
				url : $("#path",parent.document).val()+"/newsAdmin/deleteNews?deleteIds=" + gr,
				type : 'POST',
	...
	$("#newsFileBn").click(function() {
		if (checkFile()) {
			...
			$.ajaxFileUpload({
				url : $("#path",parent.document).val()+'/newsAdmin/impNewsAll', // 需要链接到服务器地址
				secureuri : false,
...
function load_news_category() {
	$.post($("#path",parent.document).val()+"/newsAdmin/findNewsCategorys", function(json) {
		if (json.result == true) {
