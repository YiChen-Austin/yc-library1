$(document).ready(function(){
    $("#detailBusinessDictionary").validationEngine({
...
$(function(){
    ...
    jQuery("#childList").navGrid('#pageList', {
        ...
        onClickButton: function(){
            rowNum = jQuery('#childList').jqGrid('getGridParam', 'rowNum') + _count++;
            var rowId = rowNum + 1;
            // 加入新增的rowId
            newIdsStr = newIdsStr.concat(';' + rowId.toString() + ',');
            _newIdsStr = _newIdsStr.concat(';' + rowId.toString() + ',');
    ...
    $("#resetButton").click(function(){
        ...
        $("remark").attr("value", "");
        // 清除grid的数据
        jQuery("#childList").jqGrid('clearGridData');
    });
...
function submitAjax(ids){
    ...
    var data = "cnName=" + cnName + "&enName=" + enName + "&flag=" + flag + "&remark=" + remark;
    $.each(ids, function(i){
        var userCell = jQuery("#childList").jqGrid('getCell', ids[i], 'action');
        if (add_key == userCell) {
	...
	// 没有添加明细
    if (0 == insertVal.length) {
    	jAlert(msg2);
    }
    else {
        var values = insertVal;
        $.ajax({
            type: "POST",
            cache: false,
            async: false,
            dataType: "json",
            type: "POST",
            url: $("#path",parent.document).val()+"/admin/"+"saveBusinessDictionary",
