...
public class PayDataService {
	...
	@Autowired
	private WebMemberDepositService webMemberDepositService;//资金
	@Autowired
	...
	public String payResponse(String orderId, String status) throws Exception {
		...
		return payResponse4Game(orderId, status, null);
	}
	 ...
	 */
	@Transactional
	public String payResponse(String orderId, String status, String thridOrderSn) throws Exception {
		...
		// 1、处理订单状态
		String orderNo = "";
		List<PayOrders> lPays = payOrdersDao.findPayOrdersByPay(payPri.getPrimaryId());
		for (PayOrders pOrder : lPays) {
			// 普通订单
			// if ("00".equalsIgnoreCase(pOrder.getOrderType())) {
			// 修改订单状态
			Order order = orderDao.findOrderById(pOrder.getOrderId());
			orderNo += orderNo.length() > 0 ? "," + order.getOrderSn() : order.getOrderSn();
	 ...
	 */
	@Transactional
	public String payResponse4Game(String orderId, String status, String thridOrderSn) throws Exception {
		List<PayPrimary> list = payPrimaryDao.findPayPrimaryBySn(orderId);
		if (list.size() <= 0)
			throw new Exception("no orders.");
		PayPrimary payPri = list.get(0);
		...
		for (PayOrders pOrder : lPays) {
			...
			//支付后处理
			//1、充值
			if(MallEnum.GoodsType.recharge2account.getIndex().equalsIgnoreCase(order.getType())){
			    webMemberDepositService.plusBalance4Recharge(order.getBuyerId(), order.getOrderAmount().doubleValue());
			}
			//2、游戏通知
			else if(MallEnum.GoodsType.consumption2product.getIndex().equalsIgnoreCase(order.getType())
	...
	public List<BankPayVo> findBankPays() {
		...
		for (PayBanks payBank : list) {
			...
			// 2、第三方接口信息
//			PayBank2ThirdEx pbtEx = payBank2ThirdExDao.findPayBank2ThirdEx(payBank.getId());
//			third2PayUtil(pbtEx, vo);
	 ...
	 */
	@Transactional
	public void saveYzmPayInteraction(PayInteraction entity) throws FrameworkException {
	 ...
	 */
	@Transactional
	public void updatePayPrimary(PayPrimary entity) throws FrameworkException {
	...
	@Transactional
	public void savePayOrders(List<GeneralOrder> list, String orderType, PayPrimary pay) throws FrameworkException {
	...
	public boolean validOrders(String orderNo, String orderType, String amount) {
		try {
			...
			for (String order : orders) {
				GeneralOrder o = "00".equalsIgnoreCase(orderType) ? orderService.getOrderBySn(order) : null;
				// 无效订单号（不存在订单）
				if (o == null) {
					return false;
				}
	...
	public void third2PayUtil(PayBank2ThirdEx pbtEx, BankPayVo bankPayVo) {
		...
		if (!BaseUtil.isEmpty(pbtEx.gettPayCommonDebit())) {
			sb.append("'commonDebit':{'tid':'" + pbtEx.gettPayCommonDebit().getId()
					+ "','tBankCode':'" + payBank2ThirdDao
							.findPayBank2Third(bankPayVo.getId(), pbtEx.gettPayCommonDebit().getId()).getDebit()
					+ "'}");
		...
		if (!BaseUtil.isEmpty(pbtEx.gettPayCommonCredit())) {
			if (sb.length() > 0)
				sb.append(",");
			sb.append("'commonCredit':{'tid':'" + pbtEx.gettPayCommonCredit().getId()
					+ "','tBankCode':'" + payBank2ThirdDao
							.findPayBank2Third(bankPayVo.getId(), pbtEx.gettPayCommonCredit().getId()).getCredit()
					+ "'}");
		...
		if (!BaseUtil.isEmpty(pbtEx.gettPayQuickDebit())) {
			if (sb.length() > 0)
				sb.append(",");
			sb.append(
					"'quickDebit':{'tid':'" + pbtEx.gettPayQuickDebit().getId()
							+ "','tBankCode':'" + payBank2ThirdDao
									.findPayBank2Third(bankPayVo.getId(), pbtEx.gettPayQuickDebit().getId()).getDebit()
							+ "'}");
		...
		if (!BaseUtil.isEmpty(pbtEx.gettPayQuickCredit())) {
			if (sb.length() > 0)
				sb.append(",");
			sb.append("'quickCredit':{'tid':'" + pbtEx.gettPayQuickCredit().getId()
					+ "','tBankCode':'" + payBank2ThirdDao
							.findPayBank2Third(bankPayVo.getId(), pbtEx.gettPayQuickCredit().getId()).getCredit()
					+ "'}");
